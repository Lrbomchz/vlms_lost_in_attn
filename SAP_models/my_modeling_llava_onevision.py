...
class LlavaOnevisionModel(LlavaOnevisionPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.language_model = MyQwen2Model(config.text_config)
        self.pad_token_id = self.config.pad_token_id if self.config.pad_token_id is not None else -1

        # ====== 运行时可改的配置（对齐你在 LLaVA-1.5 的做法）======
    ...
    def get_image_features(
        ...
        image_features = self.vision_tower(pixel_values, output_hidden_states=True, output_attentions=output_vision_attentions)
        # If we have one vision feature layer, return the corresponding hidden states,
        # otherwise, select the hidden states of each feature layer and concatenate them

        if output_vision_attentions and getattr(image_features, "attentions", None) is not None:
            self.vision_attentions = (image_features.attentions[-1],)
        else:
            self.vision_attentions = None
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if pixel_values is not None:
            ...
            image_features = self.get_image_features(
                ...
                output_vision_attentions=need_vision_attn and output_vision_attentions,
            )
        ...
        if aligned_prob is not None and vtr is not None and hasattr(self.language_model, "layers"):
            for idx, layer in enumerate(self.language_model.layers):
                ...
                # ✅ 这里按你需要挑层（你之前是 idx not in [...] continue）
                # 例：只注入 [18..23]
                #if idx not in [18, 19, 20, 21, 22, 23]:
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if cache_position[0] == 0:
            ...
            # prefill 才允许跑/返回 vision attentions
            model_inputs["output_vision_attentions"] = bool(output_vision_attentions)
        else:
            # decode 强制关闭（避免误传 / 避免 vision tower 复跑）
